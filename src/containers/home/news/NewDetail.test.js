...
describe('NewsDetail Component', () => {
    it('shows "Artikel tidak ditemukan" if no article in state', () => {
        render(_jsx(MemoryRouter, { initialEntries: ['/news/1'], children: _jsx(Routes, { children: _jsx(Route, { path: "/news/:id", element: _jsx(NewsDetail, {}) }) }) }));
    ...
    it('renders article details and recommended articles', async () => {
        ArticleData.getDataBusiness.mockResolvedValue({
            data: { articles: mockRecommended },
        });
        render(_jsx(MemoryRouter, { initialEntries: [{ pathname: '/news/1', state: { article: mockArticle } }], children: _jsx(Routes, { children: _jsx(Route, { path: "/news/:id", element: _jsx(NewsDetail, {}) }) }) }));
        // Tunggu render utama
        await waitFor(() => {
